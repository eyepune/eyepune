-- ============================================================
-- EyE PunE — COMPLETE DATABASE SETUP
-- Run this SQL in the Supabase SQL Editor (supabase.com → SQL Editor)
-- This is a combined, idempotent script safe to run multiple times.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS user_profiles (
  ...
  phone TEXT, company TEXT, address TEXT, city TEXT, state TEXT,
  pincode TEXT, gst_number TEXT, bio TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS bookings (
  ...
  duration TEXT DEFAULT '30 min', type TEXT DEFAULT 'consultation',
...
CREATE TABLE IF NOT EXISTS activities (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  lead_id UUID REFERENCES leads(id),
  type TEXT NOT NULL, description TEXT, performed_by TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS client_projects (
  ...
  budget DECIMAL(10,2), paid_amount DECIMAL(10,2) DEFAULT 0,
  description TEXT, services TEXT[], progress INTEGER DEFAULT 0, notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS client_files (
  ...
  file_type TEXT, file_size INTEGER, uploaded_by TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS onboarding_tasks (
  ...
  assignee TEXT, due_date DATE, completed_date DATE,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS onboarding_progress (
  ...
  current_step INTEGER DEFAULT 0, completed_steps INTEGER DEFAULT 0, total_steps INTEGER DEFAULT 5,
...
CREATE TABLE IF NOT EXISTS time_logs (
  ...
  user_email TEXT NOT NULL, date DATE NOT NULL,
  hours DECIMAL(5,2) NOT NULL, description TEXT,
...
CREATE TABLE IF NOT EXISTS client_messages (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  project_id UUID REFERENCES client_projects(id) ON DELETE CASCADE,
  sender_email TEXT NOT NULL, message TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS testimonials (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  customer_name TEXT NOT NULL, customer_title TEXT,
  customer_company TEXT, customer_image TEXT,
  content TEXT NOT NULL,
  rating INTEGER CHECK (rating BETWEEN 1 AND 5),
  status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'approved', 'rejected')),
  featured BOOLEAN DEFAULT FALSE, service TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS client_logos (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  company_name TEXT NOT NULL, logo_url TEXT NOT NULL,
  website_url TEXT,
...
CREATE TABLE IF NOT EXISTS payments (
  ...
  amount DECIMAL(10,2) NOT NULL, currency TEXT DEFAULT 'INR',
  status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'completed', 'failed', 'refunded')),
  payment_method TEXT, transaction_id TEXT, description TEXT, plan_name TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS contracts (
  ...
  scope_of_work TEXT, deliverables TEXT, terms TEXT,
  value DECIMAL(10,2),
  status TEXT DEFAULT 'draft' CHECK (status IN ('draft', 'sent', 'signed', 'expired', 'cancelled')),
  signed_date TIMESTAMPTZ, expiry_date DATE,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS proposals (
  ...
  valid_until DATE, viewed_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS invoices (
  ...
  due_date DATE, paid_date TIMESTAMPTZ, notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS employee_agreements (
  ...
  start_date DATE, salary DECIMAL(10,2),
...
CREATE TABLE IF NOT EXISTS offer_letters (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  candidate_name TEXT NOT NULL, candidate_email TEXT NOT NULL,
  title TEXT NOT NULL, content TEXT,
  job_title TEXT, department TEXT,
  start_date DATE, salary DECIMAL(10,2),
  status TEXT DEFAULT 'draft' CHECK (status IN ('draft', 'sent', 'signed', 'expired')),
...
CREATE TABLE IF NOT EXISTS document_templates (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name TEXT NOT NULL, document_type TEXT NOT NULL,
  content TEXT, variables TEXT[],
  is_active BOOLEAN DEFAULT TRUE, usage_count INTEGER DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS crm_sync_configs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name TEXT NOT NULL, crm_type TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS email_sequences (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name TEXT NOT NULL, steps JSONB DEFAULT '[]',
...
CREATE TABLE IF NOT EXISTS shared_documents (
  ...
  title TEXT NOT NULL, content TEXT,
  created_by TEXT NOT NULL,
  is_locked BOOLEAN DEFAULT FALSE, locked_by TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS resource_allocations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  project_id UUID REFERENCES client_projects(id) ON DELETE CASCADE,
  user_email TEXT NOT NULL, role TEXT,
...
BEGIN
  ...
  IF admin_count > 0 THEN
    RETURN jsonb_build_object('success', false, 'message', 'Admin users already exist.');
  END IF;
  
  SELECT EXISTS(SELECT 1 FROM users WHERE id = target_user_id), email
  INTO user_exists, user_email FROM users WHERE id = target_user_id;
  ...
  RETURN jsonb_build_object('success', true, 'message', 'Promoted to admin.', 'email', user_email);
END;
$$;
